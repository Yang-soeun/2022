...
/*
* 프로그램이 목적 및 기본기능: 파일 읽어서 화면 출력하기
* 프로그램 작성자: 22012212 양소은
* 프로그램 작성일: 2022.01.11(화)
*/
...
BOOL CMFCApplication2App::InitInstance()
{
	...
	//여기에 코드를 추가
	//MFCApplication2::InitInstance()함수에서 텍스트 파일을 읽어 파일 내 문자열을
	//CString 클래스 객체로 보관한다.
	FILE *file = fopen("sample.txt", "r");//파일을 읽기 모드로 열기
	...
	char str[100];
	fgets(str, 100, file);//파일에서 문자열을 읽어와서 str에 저장
	theApp.text = str;
